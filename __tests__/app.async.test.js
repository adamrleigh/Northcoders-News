  ...
  describe("Invalid post requests", () => {
    test("If insufficient arguments: status 400", async () => {
      ...
      expect(message).toEqual(
        'Bad Request error: null value in column "slug" violates not-null constraint'
      );
    });
...
describe("GET /api/articles", () => {
  ...
  test("Results can be limited by limit query", async () => {
    const {
      body: { articles },
    } = await request(app).get("/api/articles?limit=2").expect(200);
...
describe("GET /api/articles/:article_id", () => {
  ...
  describe("Bad article_id parameter", () => {
    test("if article_id is valid but not found: Status 404", async () => {
      const {
        body: { message },
      } = await request(app).get("/api/articles/1000").expect(404);
    });

    test("if article_id is invalid: Response 400 ", async () => {
      const {
        body: { message },
      } = await request(app).get("/api/articles/bad").expect(400);
    });
  });
});
...
describe("GET /api/articles/:article_id/comments", () => {
  ...
  test("Pagination works for comments", async () => {
    const {
      ...
      .get(
        "/api/articles/1/comments?sort_by=comment_id&&order=asc&&limit=3&&p=2"
      )
      .expect(200);
  ...
  test("If article_id is valid but article not found: Response 404", async () => {
    const {
      body: { comments },
    } = await request(app).get("/api/articles/1000/comments").expect(404);
  });
  test("If article_id is invalid: Response 400 and returns array containing comment objects", async () => {
    const {
      body: { comments },
    } = await request(app).get("/api/articles/egg/comments").expect(400);
  });
});
...
describe("PATCH /api/articles/:article_id", () => {
  test("If article_id exists and inc_votes is positive: Status 200 and returns updated article", async () => {
    ...
    const {
      body: { article },
    } = await request(app).patch("/api/articles/1").send(newObj).expect(200);
  ...
  test("If article_id exists and inc_votes is supplied but is not a number: Status 400", async () => {
    ...
    const {
      body: { article },
    } = await request(app).patch("/api/articles/1").send(newObj).expect(400);
  });
  test("If valid id supplied but not found: Status 404", async () => {
    newObj = { inc_votes: -100 };
    const {
      body: { article },
    } = await request(app).patch("/api/articles/100").send(newObj).expect(404);
  });
});
...
describe("PATCH /api/comments/:comment_id", () => {
  test("If comment exists and inc_votes is positive: Status 200 and returns updated article", async () => {
    ...
    const {
      body: { comment },
    } = await request(app).patch("/api/comments/1").send(newObj).expect(200);
  ...
  test("If comment exists and inc_votes is supplied but is not a number: Status 400", async () => {
    ...
    const {
      body: { article },
    } = await request(app).patch("/api/comments/1").send(newObj).expect(400);
  });
  ...
  test("If valid id supplied but not found: Status 404", async () => {
    ...
    const {
      body: { article },
    } = await request(app).patch("/api/comments/1000").send(newObj).expect(404);
  });
});
...
describe("DELETE /api/comments/:comment_id", () => {
  ...
  test("If article_id invalid: Status 400", () => {
    return request(app).delete("/api/comments/bad").expect(400);
  });
});
...
describe("GET /api/users/:username", () => {
  ...
  test("If username does not exist: Status 404", async () => {
    const {
      body: { user },
    } = await request(app).get("/api/users/beserker").expect(404);
  });
});
...
describe("POST /api/articles", () => {
  test("If article includes compulsory fields: Status 201 and posted article returned", async () => {
    ...
    const {
      body: { article },
    } = await request(app).post("/api/articles").send(newArticle).expect(201);
  ...
  test("If too few arguments supplied: Status 400", async () => {
    ...
    const {
      body: { message },
    } = await request(app).post("/api/articles").send(newArticle).expect(400);

    expect(message).toBe(
      'Bad Request error: null value in column "body" violates not-null constraint'
    );
  });
...
describe("DELETE api/articles/:article_id", () => {
  ...
  test("If valid ID but article does not exist: Status 404", () => {
    return request(app).delete("/api/articles/1000").expect(404);
  });
  test("If invalid ID: Status 400", () => {
    return request(app).delete("/api/articles/adam").expect(400);
  });
});
...
describe("GET /api/comments/:comment_id", () => {
  ...
  test("If valid id but comment does not exist: Status 404", async () => {
    const {
      body: { comment },
    } = await request(app).get("/api/comments/1000").expect(404);
  });
  test("If invalid id: Status 400", async () => {
    const {
      body: { comment },
    } = await request(app).get("/api/comments/bad").expect(400);
  });
});
...
describe("GET /api/users/:username/comments", () => {
  test("If user exists: Status 200 and returns comment", async () => {
    const {
      body: { comments },
    } = await request(app).get("/api/users/icellusedkars/comments").expect(200);
  ...
  test("If valid user but has no comments: Status 200 return empty array", async () => {
    const {
      body: { comment },
    } = await request(app).get("/api/users/lurker/comments").expect(200);
  });
  test("If valid username but no user exists: Status 404", async () => {
    const {
      body: { comment },
    } = await request(app).get("/api/users/valid/comments").expect(404);
  });
});
...
describe("GET /api/topics/:slug", () => {
  test("If topic exists: Status 200 and returns topic", async () => {
    const {
      body: { topic },
    } = await request(app).get("/api/topics/cats").expect(200);
  ...
  test("If topic does not exist: Status 404", async () => {
    const {
      body: { comment },
    } = await request(app).get("/api/topics/bbq").expect(404);
  });
});
...
describe("DELETE /api/topics/:slug", () => {
  ...
  test("If topic does not exist: Status 404", () => {
    return request(app).delete("/api/topics/bbq").expect(404);
  });
});
...
describe("DELETE /api/users/:username", () => {
  ...
  test("If user does not exist: Status 404", () => {
    return request(app).delete("/api/users/adam").expect(404);
  });
});
